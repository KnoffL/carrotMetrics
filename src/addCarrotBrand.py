...
def addCarrotBrand():
    data = getData("carrotBrands")

    # Get column names as list
    columns = data.columns.values.tolist()
    
    # Create empty list for new carrot brand
    new_carrot_brand = list()

    # Iterate over columns and request user input for each one
    for column in columns:
        print("Enter value for " + column)
        value = input()
        new_carrot_brand.append(value)
    
    print("You entered the following: ")
    print(new_carrot_brand)
    ...
    if answer == "y":
        ...
        try:
            ...
            data.loc[len(data)] = new_carrot_brand

            # Save to csv
            data.to_csv("data/carrotBrands.csv", index=False)
            print("Saved!")
        except ValueError:
            if columns == []:
